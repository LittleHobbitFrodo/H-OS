...
#pragma once
...
#ifndef H_OS_LIB_EXCEPTIONS_H
	#define H_OS_LIB_EXCEPTIONS_H

	typedef struct exception_stack_frame {
		u32 exception_type;	//	pushed second
		u32 index;	//	pushed first (by irq or additionally by wrapper (if not by irq))
		u64 rip;
		u64 cs;
		u64 rflags;
		stack_frame* rsp;
	...
	__attribute__((target("general-regs-only"))) extern void handle_exception(exception_stack_frame* frame);
