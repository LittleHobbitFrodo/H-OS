	...
	namespace mem {
		namespace page {
			...
			typedef struct table {
				struct mem::page::entry entries[512];
			} __attribute__((aligned(0x1000))) table;

			typedef struct table_array {
				const static u32 count {4};
				struct mem::page::table tbls[4];

				void clean();
				void init();

			} __attribute__((aligned(4096))) table_array;

			extern "C" mem::page::table_array* find();


			static struct mem::page::table_array* tables {nullptr};

			const static u32 entries_total {512*4};

			const static u64 page_size {4096};

			//extern "C" void change(mem::page::table_array* ptr);
