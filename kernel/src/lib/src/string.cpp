...
//
		...
		inline i8 string::cmp(const char *str) {
		...
		inline i8 string::cmp(const string &str) {
			if (this->len < str.len) {
				return -1;
		...
		inline void string::take_over(string &str) {
			//output << out::change(out::col::yellow) << "taking over " << &str << out::change() << out::endl;
			if (str.cto) {
				if (this->dt != nullptr) {
					mem::heap::free(this->dt);
				}
